...
public final class RendezvousClient implements RawPacketListener {
    ...
    // Host-role only (Phase 5) — the joinerAccountId attached to each peer's most recent
    ...
    // null = anonymous host (unchanged pre-Phase-4 behavior). Set only via the account-aware
    ...
    // Stops both the REGISTER/JOIN retry loop and — more importantly — the keepalive loop.
    // Without this, the keepalive thread would keep sending REGISTER every
    // KEEPALIVE_INTERVAL_MILLIS forever (for as long as the process lives), even after the
    // host has actually closed their world — which is exactly how this method came to exist.
    @Override
    public void cancel() {
        state = State.DONE;
    }
    ...
    /** As {@link #joinRoom(String, MatchCallback)}, but attaches the joiner's account so the host can identify them for save-data isolation (Phase 5). */
    ...
    public void onPacket(byte[] data, int length, InetAddress address, int port) {
        if (!address.equals(rendezvousAddress) || port != rendezvousPort) {
            // Expected in normal operation (peer PUNCH traffic is PunchCoordinator's concern,
            // not ours), but not logging it at all would mean losing the trail if something
            // unexpected ever lands here instead (e.g. a peer's PUNCH arriving before
            // P2PBridge has switched rendezvousListener over to PunchCoordinator).
